...
export  function getItems(options?: { pendingShippingOnly?: boolean }) {
...
export  function getItemsByCategory(genre: string, character: string) {
  const items = db.item.findMany({
...
export  function getItemsByMaker(maker: string) {
  const items = db.item.findMany({
    where: { maker },
    orderBy: { createdAt: "desc" },
    include: { sales: SALES_FOR_CARD },
...
// 월 단위로만 아는(shipDateApprox) 품목은 그 달 1일로 저장돼 있으므로, 기본 날짜 정렬만으로는
...
export  function getPurchasesInMonth(year: number, month: number, genre?: string) {
...
// 별도 목록에서만 보여준다).
export  function getShipmentsOnDate(dateStr: string) {
