...
export default  function ShipmentsPage({
  ...
  const { view: viewParam, month: monthParam, date: dateParam } = searchParams;
 ...
 function CalendarView({ monthParam }: { monthParam?: string }) {
  ...
  const allItems = getShipmentsInMonth(year, month);
 ...
 function ListView() {
  const items = getUpcomingShipments();
 ...
 function DayView({ dateStr }: { dateStr: string }) {
  const items = getShipmentsOnDate(dateStr);
