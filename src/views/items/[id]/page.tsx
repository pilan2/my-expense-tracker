...
export default  function ItemDetailPage({
  ...
  const { id } = params;
  const { from: fromParam, mode } = searchParams;
  const from = safeRedirectTarget(fromParam ?? "/items");
  const isEditing = mode === "edit";

  const [item, genreCatalog, itemTypeCatalog, sales] = [
    getItem(id),
    getGenreCatalog(),
    getItemTypeCatalog(),
    getSalesForItem(id),
  ...
  return (
    <div className="box-border mx-auto w-full max-w-xl overflow-x-hidden p-6">
              ...
              value={
                item.isPhysical ? (
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <LocalForm action={confirmDelivery.bind(null, item.id)}>
      ...
      <LocalForm action={deleteItem.bind(null, item.id, from)} className="mt-8 border-t pt-6 dark:border-neutral-800">
        <ConfirmSubmitButton
      ...
      <div className="mt-8 border-t pt-6 dark:border-neutral-800">
        ...
        {remaining > 0 ? (
          <LocalForm action={createSale.bind(null, item.id)} className="mb-6 flex flex-col gap-3">
          ...
          <ul className="divide-y divide-neutral-200 text-sm dark:divide-neutral-800">
            {sales.map((sale) => (
              <li key={sale.id} className="flex items-center justify-between py-2">
                ...
                <LocalForm action={deleteSale.bind(null, sale.id)}>
                  <ConfirmSubmitButton
