...
export default  function EventDetailPage({ params }: { params: { id: string } }) {
  const { id } = params;
  const [event, itemOptions] = [getEvent(id), getItemOptions()] as const;
            ...
            <div key={booth} className="rounded-lg border border-neutral-200 p-4 dark:border-neutral-800">
              ...
              <ul className="flex flex-col gap-2">
                {entries.map((entry) => {
                  ...
                  >
                    <LocalForm action={toggleChecklistItem.bind(null, entry.id)}>
                      <button
                        type="submit"
                    ...
                    <LocalForm action={deleteChecklistItem.bind(null, entry.id)}>
                      <ConfirmSubmitButton
